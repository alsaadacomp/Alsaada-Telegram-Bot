...
export const viewCurrentEmployeesHandler = new Composer<Context>()

// معالج عرض العاملين الحاليين
viewCurrentEmployeesHandler.callbackQuery(/^hr:employees:view-current(:\d+)?$/, async (ctx) => {
  ...
  try {
    ...
    if (currentPageEmployees.length === 0) {
      ...
      await ctx.editMessageText(
        '👷 عرض العاملين الحاليين\n\n❌ لا يوجد عاملين حالياً في النظام.',
    ...
    if (page > 0) {
      navigationRow.push({
        text: '⬅️ السابق',
        callback_data: `hr:employees:view-current:${page - 1}`
      })
    }
    ...
    await ctx.editMessageText(
      '❌ حدث خطأ في تحميل بيانات العاملين الحاليين.\n\n' +
